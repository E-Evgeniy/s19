...
export default function ItemOrderTable(props) {
    ...
    const ROOT_ADDRESS = 'item_order_units'
            ...
            <Modal
                ...
                <div className="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
                    <div className="bg-white px-16 py-14 rounded-md text-center">
                        <h1 className="text-xl mb-4 font-bold text-slate-500">{t('description.item_order_delete')} {currentObject} {t('description.item_order_delete_1')}</h1>
                        <button
                        ...
                        {props.loadedObjects.map((object, index) => {
                            return (
                                <tr key={object.id} className={needHide(object.id)}>
                                    <td className="p-2 border-r w-1/5">{object.name}</td>
                                    <td className="p-2 border-r w-1/5">{object.casting_volume}</td>
                                    <td className="p-2 border-r w-1/5">
                                        {object.matrix_name}
                                    </td>
                                    <td className="p-2 border-r w-1/5">{object.matrix_volume}</td>                                    
