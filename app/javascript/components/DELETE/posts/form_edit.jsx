...
export default function FormEdit() {
  ...
  useEffect(() => {
    ...
    let location = useLocation().pathname.split(`${ROOT_ADDRESS}s/`)[1];
    ...
    const handleSubmit = async (event) => {
        ...
        formData.append("post[name]", finalValue(inputName, countEditName, editName));
        formData.append("post[description]", finalValue(inputDescription, countEditDescription, editDescription));
        ...
        try {
            await axios.patch(`/api/v1/${ROOT_ADDRESS}s/${location}`, formData, {
                headers: {
                    'X-CSRF-TOKEN': csrfToken
                }
            });
            navigate(`/${ROOT_ADDRESS}s/${location}`);
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const apiEndpoint = `/api/v1/${ROOT_ADDRESS}s/${location}`

        fetch(apiEndpoint)
            .then(response => response.json())
            .then(data => {
                setEditName(data["post"]["name"])
                setInputName(data["post"]["name"])
                        ...
                        <div className="mb-5">
                            <div className="mb-3 block text-base font-medium text-[#07074D]">
                                ...
                                <input
                                    ...
                                    type={t('description.post_name')}
                                    name={t('description.post_name')}
                                    value={finalValue(inputName, countEditName, editName)}
                                    onChange={handleNameChange}
                                    placeholder={t('description.enter_post_name')}
