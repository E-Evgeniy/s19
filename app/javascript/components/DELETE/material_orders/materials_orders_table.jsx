...
export default function MaterialsOrdersTable(props) {
            ...
            <Modal
                ...
                <div className="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
                    <div className="bg-white px-16 py-14 rounded-md text-center">
                        <h1 className="text-xl mb-4 font-bold text-slate-500">{t('description.material_order_delete')} {currentObject}</h1>
                        <button
                        ...
                        {props.loadedObjects.map((object, index) => {
                            return (
                                <tr key={object.id} className={needHide(object.id)}>
                                    <td className="p-2 border-r w-1/5">{object.supplier_name}</td>
                                    <td className={classTdLink}>
                                        <NavLink to={String(object.id)}>{object.contract_name}</NavLink>
                                    </td>
                                    <td className="p-2 border-r w-1/5">{object.data_order}</td>
                                    <td className="p-2 border-r w-1/5">{object.total_price}</td>
