...
export default function MaterialsOrderTable(props) {
    ...
    const RequestDeleteObject = async (id) => {
        await fetch(`/api/v1/material_order_items/${id}`, {
            method: 'DELETE',
        }).then((response) => {
            if (response.ok) {
            ...
            <Modal
                ...
                <div className="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
                    <div className="bg-white px-16 py-14 rounded-md text-center">
                        <h1 className="text-xl mb-4 font-bold text-slate-500">{t('description.delete_supplier')} {currentObject}</h1>
                        <button
                        ...
                        {props.loadedObjects.map((object, index) => {
                            return (
                                <tr key={object.id} className={needHide(object.id)}>
                                    ...
                                    <td className="p-2 border-r w-1/6">{object.delivery_period}</td>
                                    <td className="p-2 border-r w-1/6">{object.price}</td>
                                    <td className="p-2 border-r w-1/6">{object.massa}</td>
                                    <td className="p-2 border-r w-1/6">{object.total_price}</td>
