...
export default function materialTable(props) {
    ...
    const RequestDeleteObject = async (id) => {
        await fetch(`/api/v1/materials/${id}`, {
            method: 'DELETE',
        }).then((response) => {
            if (response.ok) {
            ...
            <Modal
                ...
                <div className="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
                    <div className="bg-white px-16 py-14 rounded-md text-center">
                        <h1 className="text-xl mb-4 font-bold text-slate-500">{t('description.material_delete')} {currentObject}</h1>
                        <button
                ...
                <table className="w-full h-full" style={{ tableLayout: 'fixed' }}>
                    ...
                    <tbody>
                        <tr key={props.loadedObject.id} className={needHide(props.loadedObject.id)}>
                            <td className="p-2 border-r w-1/4">{props.loadedObject.name}</td>
                            <td className="p-2 border-r w-1/4">{props.loadedObject.average_price}</td>
                            <td className="p-2 border-r w-1/4">{props.loadedObject.description}</td>
