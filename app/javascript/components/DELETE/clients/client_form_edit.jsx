...
export default function ClientFormEdit() {
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (event) => {
        ...
        formData.append("client[name]", finalValue(inputName, countEditName, editName));
        formData.append("client[inn]", finalValue(inputInn, countEditInn, editInn));
        formData.append("client[address]", finalValue(inputAddress, countEditAddress, editAddress));
        formData.append("client[site]", finalValue(inputSite, countEditSite, editSite));
        formData.append("client[email]", finalValue(inputEmail, countEditEmail, editEmail));
        ...
        try {
            await axios.patch(`/api/v1/clients/${location}`, formData, {
                headers: {
    ...
    useEffect(() => {
        ...
        const apiEndpoint = `/api/v1/client/find_for_edit?id=${location}&input_inn=${inputInn}&count_edit_inn=${countEditInn}`
        ...
        fetch(apiEndpoint)
            ...
            .then(data => {
                ...
                setEditInn(data["client_edit"].inn)
                setEditAddress(data["client_edit"].patronymic)
                setEditEmail(data["client_edit"].email)
                setEditSite(data["client_edit"].site)
