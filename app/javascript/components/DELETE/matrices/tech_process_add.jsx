...
export default function TechProcessAdd(prop) {
    ...
    const ROOT_ADDRESS = 'matrix_tech_process'
    const PLURAR = 'es'
    ...
    const { matrix_id, onTechProcessAdd } = prop;
    ...
    const handleSubmit = async (event) => {
        ...
        formData.append("object[matrix_id]", matrix_id);
        formData.append("object[tech_process_id]", idObject);
        ...
        setInputObjectName('')
        navigate(`/matrices/${matrix_id}`);
    };

    useEffect(() => {
        const apiEndpoint = `/api/v1/${ROOT_ADDRESS}/find_for_add_object?input_name=${inputObjectName}&matrix_id=${matrix_id}`
    ...
    if (addObjectExist == true) {
        ...
        text_error = t('description.tech_process_error_empty')
    } else {
                        ...
                        <tr className={class_tr_head_1}>
                            <th className={class_th_head_0} colSpan="2">
                                <div className={class_div_head_1}>
                                    {t('description.tech_process_add')}</div>
                            </th>
                        </tr>
                        <tr className={class_tr_head_1}>
